...
import com.nicole.server.pojo.*;
import com.nicole.server.service.*;
...
public class EmployeeController {
  ...
  public void exportEmployee(HttpServletResponse response) {
    ...
    Workbook workbook = ExcelExportUtil.exportExcel(params, Employee.class, list);
      ...
      // 中文乱码
      response.setHeader(
          "content-disposition", "attachment;filename=" + URLEncoder.encode("员工表.xls", "UTF-8"));
      ...
      list.forEach(
          employee -> {
            ...
            employee.setPoliticId(
                politicsStatuses
                    .get(
                        politicsStatuses.indexOf(
                            new PoliticsStatus(employee.getPoliticsStatus().getName())))
                    .getId());
            // 部门id
            employee.setDepartmentId(
                departments
                    .get(departments.indexOf(new Department(employee.getDepartment().getName())))
                    .getId());
            // 职称id
            employee.setJobLevelId(
                joblevels
                    .get(joblevels.indexOf(new Joblevel(employee.getJoblevel().getName())))
                    .getId());
            // 职位id
            employee.setPosId(
                positions
                    .get(positions.indexOf(new Position(employee.getPosition().getName())))
                    .getId());
          });
